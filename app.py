...
app = Flask(__name__)
app.secret_key = 'I9fmL3bTcb0qB62eWhRsNHsX3Qddi6xD'
...
GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxauWibihWMUIuu70jJoL1Q0z57sPUjm-I91y2UXCk05bEKdOkJkbHCUcHuVYH5hBcQAw/exec"
...
segment_thresholds = {
    1: {"threshold": 4, "program": "ICARE", "highest_score": 12},
    2: {"threshold": 4, "program": "ICARE", "highest_score": 6},
    3: {"threshold": 5, "program": "ICARE", "highest_score": 10},
    4: {"threshold": 5, "program": "ICARE", "highest_score": 6},
    5: {
        "threshold": 4, 
        "program": "LEAP",
        "highest_score": 12,
    ...
    7: {"threshold": 8, "program": "ICARE", "highest_score": 12},
    8: {"threshold": 2, "program": "Hulagpos", "highest_score": 5}
}
...
@app.before_request
def require_login():
    allowed_routes = {'login', 'login_google', 'authorize', 'static', 'direct_pdf_download', 'webview_download_pdf'}
    ...
    if endpoint in allowed_routes:
        # For PDF download routes, require session but not Google login
        if endpoint in {'direct_pdf_download', 'webview_download_pdf'}:
            if 'session_id' not in session:
                logger.debug(f"Redirecting to index - missing session_id for {endpoint}")
                return redirect(url_for('index'))
            return
        return
...
def save_notes():
    try:
        ...
        # Store notes in session and mark it as modified
        session['notes'] = notes
        session.modified = True
        
        # Log successful save
        logger.debug(f"Notes saved successfully for session {session.get('session_id')}")
...
def prepare_google_sheets_data():
    ...
    try:
        # Initialize segment_answers dictionary
        segment_answers = {}
        ...
        for i in range(1, 9):
            ...
            # Education and Employment (Segment 5)
                ...
                # Calculate education and employment scores separately
                edu_indices = segment_thresholds[5]['education']['questions']
                emp_indices = segment_thresholds[5]['employment']['questions']
                
                # Calculate scores using the actual values from segment_scores
                education_score = sum(segment_scores[j] for j in range(len(segment_scores)) if j in edu_indices)
                employment_score = sum(segment_scores[j] for j in range(len(segment_scores)) if j in emp_indices)
                
                # Store the scores in subtotals for template access
                ordered_data[f"Segment {i} Education Score"] = str(education_score)
                ordered_data[f"Segment {i} Employment Score"] = str(employment_score)
                total_score = education_score + employment_score
                
                # Store segment answers for education/employment score calculation
                segment_answers[5] = {
                    'questions': segment_questions[5],
            ...
            ordered_data[f"Segment {i} Total"] = str(total_score)
        ...
        ordered_data["Grand Total"] = str(total_score)  # Add Grand Total field for Google Sheets
...
def results():
    ...
    # Check if we should use previous data
    use_previous_data = session.get('use_previous_data', False)
    ...
    for i in range(1, 9):
        ...
        if i == 5:  # Split segment - Education and Employment
            # Calculate education and employment scores separately
            edu_indices = segment_thresholds[5]['education']['questions']
            emp_indices = segment_thresholds[5]['employment']['questions']
            ...
            # Store the scores in subtotals for template access
            subtotals[5] = education_score
            subtotals[6] = employment_score
            total_score += education_score + employment_score
            ...
            # For all other segments, sum all scores
            subtotal = sum(segment_scores)
            subtotals[i] = subtotal
            total_score += subtotal
            
            # Store segment answers
            segment_answers[i] = {
                'questions': segment_questions[i],
                'scores': segment_scores
            }
    
    # Get client information using appropriate keys
    segment0_key = "previous_segment0" if use_previous_data else "segment0"
    notes_key = "previous_notes" if use_previous_data else "notes"
    active_session_id = session.get('previous_session_id' if use_previous_data else 'session_id', '')
    
    length_of_sentence = session.get(segment0_key, {}).get('length_of_sentence', '2-years-or-less')
...
def generate_pdf():
    try:
        ...
        # First, collect all scores - keep all segments intact
        for i in range(1, 9):
            segment_scores = session.get(f'segment{i}_scores', [])
            if not segment_scores:
                logger.warning(f"No scores found for segment {i}, using default empty list")
        ...
        education_score = 0
        employment_score = 0
        if 5 in segment_answers and 'scores' in segment_answers[5]:
            edu_indices = segment_thresholds[5]['education']['questions']
            emp_indices = segment_thresholds[5]['employment']['questions']
            
            education_score = sum(segment_answers[5]['scores'][i] for i in edu_indices if i < len(segment_answers[5]['scores']))
            employment_score = sum(segment_answers[5]['scores'][i] for i in emp_indices if i < len(segment_answers[5]['scores']))
...
def direct_pdf_download():
    ...
    try:
        ...
        if 'session_id' not in session:
            logger.warning("PDF download attempted without a valid session")
            flash("Your session has expired. Please start a new assessment.")
            return redirect(url_for('index'))
        
        # Debug session state
        session_id = session.get('session_id', 'None')
        logger.debug(f"PDF download session info - Session ID: {session_id}")
        ...
        # Set a unique filename with timestamp and client name
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        safe_client_name = "".join([c if c.isalnum() else "_" for c in client_name])[:30]
        filename = f"CARAS_{safe_client_name}_{timestamp}.pdf"
        
        # Create the full path for the PDF file
        local_pdf_path = os.path.join(os.path.expanduser('~'), 'Downloads', filename)
        
        # Generate and save PDF to disk
        logger.info(f"Saving PDF to {local_pdf_path}")
        HTML(string=html).write_pdf(local_pdf_path)
        
        # Ensure the PDF is saved with proper permissions
        os.chmod(local_pdf_path, 0o644)
        
        # Restore session data
        session['notes'] = current_notes
        session.modified = True
        
        # Check if running in frozen/desktop mode
        if getattr(sys, 'frozen', False):
            # In desktop mode, try to open the PDF directly
            try:
                open_pdf(local_pdf_path)
                flash("PDF generated successfully!")
            except Exception as open_err:
                logger.error(f"Error opening PDF: {str(open_err)}")
                flash(f"PDF saved to {local_pdf_path} - please open it manually.")
        else:
            # In development mode, notify the user where the PDF is saved
            flash(f"PDF generated successfully and saved to {local_pdf_path}")
...
def open_pdf(file_path):
    """Open a PDF file using the default system PDF viewer"""
    try:
        import platform
        import subprocess
...
@app.route('/webview_download_pdf')
def webview_download_pdf():
    """Special route for pywebview to handle PDF downloads"""
    try:
        import tempfile
        import os
        ...
        # First, check if we have a valid session
        if 'session_id' not in session:
            logger.warning("Webview PDF download attempted without a valid session")
            flash("Your session has expired. Please start a new assessment.")
            return redirect(url_for('index'))
        
        # Debug session state
        session_id = session.get('session_id', 'None')
        logger.debug(f"Webview PDF download session info - Session ID: {session_id}")
        ...
        temp_download_dir = os.path.join(tempfile.gettempdir(), 'risk_assessment_tool')
        os.makedirs(temp_download_dir, exist_ok=True)
        
        # Set a unique filename with timestamp and client name
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        safe_client_name = "".join([c if c.isalnum() else "_" for c in client_name])[:30]
        filename = f"CARAT_{safe_client_name}_{timestamp}.pdf"
        
        # Create the full path for the PDF file
        local_pdf_path = os.path.join(temp_download_dir, filename)
        ...
        logger.error(f"Error in webview PDF download: {str(e)}")
        flash("Error generating PDF. Please try again.")
        return redirect(url_for('results'))
...
def add_header(response):
    """
    Add headers to prevent caching of pages to ensure back button after logout redirects to login page.
    """
...
if __name__ == '__main__':
    ...
    if getattr(sys, 'frozen', False):
        ...
        try:
            ...
            # Set up logger for the desktop app
            logger.info("Starting Risk Assessment Tool as desktop application")
            
            # Create temp directory for PDF downloads if it doesn't exist
            import tempfile
            temp_download_dir = os.path.join(tempfile.gettempdir(), 'risk_assessment_tool')
            os.makedirs(temp_download_dir, exist_ok=True)
            ...
            logger.error(f"Error starting desktop application: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())
            # Fall back to running as web app
            app.run(debug=True)
    else:
